exports.run = (client, message, args, Discord, connection) => {
  // Needs more hugs
  var hugsAnime = ['https://media.tenor.com/images/efdd8f53689b1bb3437054d608156e95/tenor.gif',
  ...
  var hugAnime = hugsAnime[Math.floor(Math.random() * hugsAnime.length)];

  var hugsReal = ['https://media1.tenor.com/images/022a19f8ad9260b5045e16289e66c903/tenor.gif',
  ...
  var hugReal = hugsReal[Math.floor(Math.random() * hugsReal.length)];

  var lols = ['https://data.whicdn.com/images/69819901/original.gif', 'https://media1.tenor.com/images/112c2abcf585b37e6c6950ebc3ab4168/tenor.gif',
    'https://lh4.googleusercontent.com/-xxPcIb2lzAw/VIkdc2oRu7I/AAAAAAAAJrU/4bIJ8zJl9hE/w426-h239/Kill%2Bla%2Bkill%2B2.gif',
    'http://i0.kym-cdn.com/photos/images/original/000/906/455/51f.gif', 'https://media.giphy.com/media/IFqVnhA55IUdG/giphy.gif',
    'http://mrwgifs.com/wp-content/uploads/2013/09/Awkward-Hug-Gif-In-Americas-Next-Top-Model.gif', 'https://vignette.wikia.nocookie.net/teen-titans-go/images/4/49/No_Hugs.gif'];
  var lolNah = lols[Math.floor(Math.random() * lols.length)];

  var wiggyID = '232614905533038593';
  if (message.mentions.users.first()) {
    if (message.mentions.users.first().id === wiggyID && message.author.id !== '198589482058907649' && message.author.id !== '341032426799235072') {
      message.channel.send(`<@${message.author.id}> just tried to hug Lilwiggy. Yeah that didn't work.`, { file: lolNah });
      ...
      client.checkUser(message.mentions.users.first().id, message.mentions.users.first().displayAvatarURL, () => {
        connection.query(`SELECT \`weeb\` FROM \`user\` WHERE \`User_ID\` = ${message.mentions.users.first().id}`, (err, res, firlds) => {
          if (err)
            throw err;
          if (res[0].weeb === 'on') {
